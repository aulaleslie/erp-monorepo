...
export class SalesOrdersService {
  ...
  async create(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      return this.findOne(document.id, tenantId, userId);
    });
  }
  ...
  async update(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (dto.deliveryDate) header.deliveryDate = new Date(dto.deliveryDate);
  ...
  async convertToInvoice(
    ...
    if (order.status !== DocumentStatus.APPROVED) {
      throw new BadRequestException(
        'Only APPROVED orders can be converted to invoice',
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      // Copy Lines
      const items = await manager.find(DocumentItemEntity, {
        where: { documentId: orderId },
      });
      for (const item of items) {
        const newItem = manager.create(DocumentItemEntity, {
          ...item,
          id: undefined,
      ...
      // Create Sales Header for Invoice
      const orderHeader = await manager.findOne(SalesHeaderEntity, {
        where: { documentId: orderId },
      });
      if (orderHeader) {
        const invoiceHeader = manager.create(SalesHeaderEntity, {
          ...orderHeader,
          id: undefined,
          documentId: invoice.id,
          createdAt: undefined,
        ...
        await manager.save(invoiceHeader);
      }

      // Record Relation
      const relation = manager.create(DocumentRelationEntity, {
        tenantId,
        fromDocumentId: orderId,
        toDocumentId: invoice.id,
        relationType: DocumentRelationType.ORDER_TO_INVOICE,
      });
