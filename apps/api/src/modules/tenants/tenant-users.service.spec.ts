...
describe('TenantUsersService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should return paginated tenant users with user and role info', async () => {
      const tenantUsers = [
        { id: 'tu-1', tenantId: 'tenant-1', userId: 'user-1', roleId: 'role-1' },
      ] as unknown as TenantUserEntity[];
      const users = [{ id: 'user-1', email: 'test@test.com', fullName: 'Test User' }] as UserEntity[];
  ...
  describe('findOne', () => {
    it('should return tenant user with user and role info', async () => {
      const membership = { id: 'tu-1', tenantId: 'tenant-1', userId: 'user-1', roleId: 'role-1' } as unknown as TenantUserEntity;
    ...
    it('should throw NotFoundException if user not a member', async () => {
      ...
      await expect(service.findOne('tenant-1', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('create', () => {
    it('should create new user and membership', async () => {
      ...
      const newMembership = { id: 'tu-1', userId: 'user-1', tenantId: 'tenant-1', roleId: 'role-1' };
  ...
  describe('inviteExistingUser', () => {
    it('should create membership for existing user', async () => {
      const user = { id: 'user-1', email: 'existing@test.com', isSuperAdmin: false } as UserEntity;
      const role = { id: 'role-1', name: 'Editor', tenantId: 'tenant-1' } as RoleEntity;
      const membership = { id: 'tu-1', userId: 'user-1', tenantId: 'tenant-1', roleId: 'role-1' };
  ...
  describe('updateRole', () => {
    it('should update user role', async () => {
      const membership = { id: 'tu-1', roleId: 'old-role' } as unknown as TenantUserEntity;
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException if membership not found', async () => {
      ...
      await expect(service.remove('tenant-1', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('updateUser', () => {
    it('should update user profile and role', async () => {
      const membership = { id: 'tu-1', roleId: 'role-1' } as unknown as TenantUserEntity;
      const user = { id: 'user-1', email: 'old@test.com', fullName: 'Old Name' } as UserEntity;
      ...
      tenantUserRepository.save!.mockResolvedValue({ ...membership, roleId: 'role-2' });
    ...
    it('should throw ConflictException if new email already in use', async () => {
      ...
      userRepository.findOne!
        .mockResolvedValueOnce(user) // First call for getting user
